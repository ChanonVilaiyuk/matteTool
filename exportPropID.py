...
from lightTools.matteTool import objectIDExportUI as ui
...
class MyForm(QtGui.QMainWindow) : 
	def __init__(self, parent=None) : 
		...
		self.settingFile = 'O:/systemTool/python/lightTools/matteTool/settingProp.yml'
		self.settingInfo = dict()
		self.mayaInfo = mayaInfo.sceneInfo()
		self.idRange = [10000, 11000]
		self.idAssetRange = 1
		self.allAssetId = []
		self.startPadding = 1
	...
	def initFunctions(self) : 
		...
		if self.settingInfo : 
			...
			'No Setting Found, cannot continue'
	...
	def exportDatabase(self) : 
		...
		if self.ui.id_tableWidget.currentItem() : 
			...
			if not objectId in currentData.keys() : 
				result = self.writeData(objectId, assetName, user, assetPath)
